...
Puppet::Type.type(:archive).provide(:default) do
  ...
  def create
    PuppetX::Bodeco::Util.download(resource[:source], archive_filepath, :username => resource[:username], :password => resource[:password], :cookie => resource[:cookie], :tls_verify => resource[:tls_verify] ) unless checksum?
    verify_checksum
    extract
    cleanup
  end
